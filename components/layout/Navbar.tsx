        ...
        <div className="hidden items-center gap-8 md:flex">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="group relative font-body text-sm text-ink-secondary transition-colors hover:text-ink"
            >
              {item.label}
              <span className="absolute -bottom-0.5 left-0 h-px w-0 rounded-full bg-accent transition-all duration-300 group-hover:w-full" />
            </a>
          ))}
        </div>
            ...
            <div className="flex flex-col gap-1 px-5 py-4">
              {navigation.map((item, i) => (
                <motion.a
                  ...
                  className="rounded-lg px-2 py-2.5 font-body text-sm text-ink-secondary transition-colors hover:bg-surface hover:text-ink"
                >
                  {item.label}
                </motion.a>
              ))}
